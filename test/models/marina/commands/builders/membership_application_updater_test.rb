...
describe Marina::Commands::Builders::MembershipApplicationUpdater do
  subject { Marina::Commands::Builders::MembershipApplicationUpdater.new user: user, data_store: data_store, mailing_list_processor: mailing_list_processor, payment_processor: payment_processor }
  ...
  it "accepts an application" do
    application.expects(:accepted_by).with(user, mailing_list_processor: mailing_list_processor, payment_processor: payment_processor)
    result = nil
  ...
  it "rejects an application" do
    application.expects(:rejected_by).with(user, reason_for_rejection: 'Stupid', mailing_list_processor: mailing_list_processor)
    result = nil
